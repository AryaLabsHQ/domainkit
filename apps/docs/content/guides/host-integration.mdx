...
`domainkit/server` supplies a catch-all Effect Layer for the shared connection mechanics. The host
provides identity, provider configuration, one-time continuations, durable connection storage, and
cryptography as sibling Layers:
...
`Server.Handler` directly inside an Effect HTTP host. An interactive provider connection has two
server routes:
