...
approval and receipt, and observed readiness. `Connect` seals provider secret material through
`Custody` before writing an authorization credential to `Storage`.
...
The eight tables and their keys are on the
...
absent rather than forbidden. `continuations.header` is the single exception, described below.
...
## The one read without a principal

`Storage.continuations.header(id)` answers with a flow's `ownerId`, `actorId`, `provider`, and
`expiresAt`, and takes no `Principal`. Writing your own implementation means writing this one
without an owner filter, on purpose: a provider callback arrives as a top-level navigation carrying
a `state` and a session cookie, so `domainkit/server` has to learn whose flow it is finishing before
it can name a principal to scope by. It then holds the host's answer against that record.

Two properties make the exception safe, and an implementation has to keep both. It returns those
four fields and never `payload`, which holds the PKCE `codeVerifier` and stays behind the
owner-scoped `get`. And it enforces the TTL exactly as `get` does, so a stale id never reaches a
host. It is a read: the row survives for `consume` to spend once the provider exchange succeeds.
`get` and `consume` keep their owner filters regardless, so a host that resolves the wrong
principal still fails closed.

## Custody

There is no plaintext mode for authorization credentials. `Connect` seals provider secret material
through `Custody` before it reaches a credential row and opens it after reading one. OAuth
continuation payloads follow a separate path: they include PKCE verifiers and CapsuleDB stores them
as unsealed JSON. Protect database access and backups, and sweep expired continuation rows.
...
`Testing.conformance.storage` checks tenant isolation and the unscoped header read, apply leases,
exactly-once continuations, revocation recovery, and lock semantics against any implementation.
