...
the provider turned down appears in `connections` as `reconnect` and contributes no zones. Each
connection carries the account `label` its provider gave at connect time, so a list names accounts
without a provider call; it is `null` when the provider named no single account.
