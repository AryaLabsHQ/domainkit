...
`isRetryable` instead of parsing text. `Messages.outcome(error, catalog, context)` picks the title
and the description; `Messages.failure` joins them into one sentence.
