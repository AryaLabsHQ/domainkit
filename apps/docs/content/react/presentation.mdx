...
plan review, records, receipts, verification, and one function per `DomainKit.Error` reason.
Overrides merge over `Messages.english`, so a host replaces one key without restating the rest.
