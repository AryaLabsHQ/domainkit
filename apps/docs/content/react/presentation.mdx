...
plan review, records, receipts, verification, and one function per `DomainKit.Error` reason. A
reason's function returns a title and a description, which is what an [outcome](/components/outcome)
draws. The
...
Nothing renders a tag. A reason becomes words through the catalog, an operation becomes a
