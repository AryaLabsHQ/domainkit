...
existing `authorizationId`, and a same-origin `returnTo`. Callback redirects append
...
without a host identity return `401`. Invalid provider input, unsafe return URLs, mismatched
callbacks, expired state, and replayed state return `400`. Provider tokens and opaque continuation
payloads are never serialized to the browser response.
