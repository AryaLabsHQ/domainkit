...
The returned `WebHandler` exposes `fetch(Request)` and `dispose()`. Create one handler during host
startup and pass each framework's standard `Request` through unchanged; do not construct a handler
for every request. The examples below assume the initialized `domainKit` is exported by the host
composition module.
...
import { domainKit } from "@/server/domainkit";

export const GET = (request: Request) => domainKit.fetch(request);
export const POST = (request: Request) => domainKit.fetch(request);
```
...
Use your host's module path in the imports above. Dispose the handler once during a long-lived
host's shutdown: this closes the ManagedRuntime created by `toWebHandler` or `createDomainKit`.
`createDomainKit` does not initialize or dispose the host-provided `persistence`; keep shared host
resources available until the handler is disposed, then complete the host's own shutdown sequence.
The default base path is `/api/domainkit`. When `baseURL` is omitted, the start request's origin
constructs the callback URL; configure it explicitly behind a reverse proxy or custom domain.

## Routes

With the default `basePath`, the handler serves these routes:
