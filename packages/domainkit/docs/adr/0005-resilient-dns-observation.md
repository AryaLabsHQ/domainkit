...
`nextCheckAt`. `observe` takes supplied `requirements` first, else the attachment's provisioning
receipt; a domain with neither is `InvalidInput`. The `verification.observe` route body and
`Transport.verification.observe(domain, { requirements })` carry the same optional field. When the attachment's provider session cannot be built, public DNS stands alone
