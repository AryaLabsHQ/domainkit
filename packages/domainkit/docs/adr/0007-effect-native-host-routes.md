...
`Server.Identity` is the only service a host must implement: a request maps to
`Principal.Shape`. Each handler derives the principal for the request it is serving and provides it
...
503 `ProviderUnavailable`, 500 for the internal reasons. Hosts that map their own routes use the
same table.
