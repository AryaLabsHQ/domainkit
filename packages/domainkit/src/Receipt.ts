...
export class Model extends Schema.Class<Model>("@domainkit/Receipt")({
  id: ReceiptId,
...
export const encode: (receipt: Model) => Encoded = Schema.encodeSync(Model);
export const isComplete = (receipt: Model): boolean => receipt.status === "complete";

/** Operations the receipt proves were written, with the provider record ids needed to undo them. */
export const applied = (receipt: Model): ReadonlyArray<Applied> =>
