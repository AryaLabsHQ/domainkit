...
export const provider = (options: Options = {}): Provider.Definition<TeamContext> => {
  ...
  const integration: Provider.IntegrationAuth | undefined =
      ...
      : {
          ...
          start: (input) => {
            const authorizationUrl = new URL(
              `https://vercel.com/integrations/${encodeURIComponent(options.integration!.slug)}/new`,
            );
          ...
          complete: (input) =>
            Effect.gen(function* () {
              const clientId = yield* resolve(
                options.integration!.clientId,
                "integration.clientId",
              );
              const clientSecret = yield* resolve(
                options.integration!.clientSecret,
  ...
  return Provider.make<TeamContext>({
    ...
    session: (credential) => {
      const options = client(Redacted.value(credential.secret));
      ...
      > =>
        teamId !== null
          ? Client.zones(options, teamId).pipe(
              Effect.map((domains) => domains.map((domain) => targetOf(domain, teamId))),
            )
          : Effect.gen(function* () {
              const personal = yield* Client.zones(options, null);
              const teams = yield* Client.teams(options).pipe(
                Effect.catchIf(
              ...
              const perTeam = yield* Effect.forEach(teams, (team) =>
                Client.zones(options, team.id).pipe(
                  Effect.map((domains) =>
      ...
      return {
        ...
        dns: (target) => {
          ...
          return Client.dns(options, scope._tag === "Some" ? scope.value.teamId : teamId);
        },
      };
    },
  });
