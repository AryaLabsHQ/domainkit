...
import type * as PromiseConnection from "../promise/connection.ts";
import * as ManagedDnsConnections from "../promise/managed-dns-connections.ts";
...
export interface AsyncIdentity {
  readonly authenticate: (request: Request) => Promise<Principal>;
}

export interface AsyncConnectionPolicy {
  readonly authorizeReuse: (input: AuthorizationReuseInput) => Promise<void>;
}

export type AsyncInteractiveFlow = PromiseConnection.InteractiveFlow;

export interface AsyncInteractiveProvider {
  readonly create: (
    input: Omit<InteractiveFlowInput, "method" | "providerId">,
  ) => Promise<AsyncInteractiveFlow>;
  readonly method: "integration" | "oauth2";
  readonly providerId: string;
}

export interface AsyncPendingAuthorizations {
  readonly consume: (id: string, now: Date) => Promise<PendingAuthorization | null>;
  readonly get: (id: string, now: Date) => Promise<PendingAuthorization | null>;
  readonly put: (pending: PendingAuthorization) => Promise<void>;
}

export interface CreateDomainKitOptions extends Configuration {
  readonly connectionPolicy: AsyncConnectionPolicy;
  readonly identity: AsyncIdentity;
  readonly pendingAuthorizations: AsyncPendingAuthorizations;
  /** Host-owned Promise persistence. DomainKit does not initialize or dispose this resource. */
  readonly persistence: ManagedDnsConnections.AsyncInterface;
  readonly providers: ReadonlyArray<AsyncInteractiveProvider>;
}
...
/** Construct the Effect-native server handler from host-provided services. */
export const make = (configuration: Configuration = {}) =>
  Effect.gen(function* () {
    ...
    const complete = Effect.fn("Server.complete")(function* (request: Request, providerId: string) {
...
/** Provide the Effect-native server handler as a Layer. */
export const layer = (configuration: Configuration = {}) =>
  Layer.effect(Handler, make(configuration));
    ...
    try: evaluate,
      ...
      cause instanceof Error
        ...
        : new Error({
            ...
            operation,
            retry: category === "storage" ? "safe" : "unknown",
          }),
  });

const asyncProvidersLayer = (
  definitions: ReadonlyArray<AsyncInteractiveProvider>,
): Layer.Layer<Providers> =>
  providersLayer(
    definitions.map((definition) => ({
      create: (input) =>
        fromPromise("Server.Providers.interactiveFlow", "provider", () =>
          definition.create(input),
        ).pipe(
          Effect.map((flow) => ({
            complete: (payload, callbackUrl) =>
              fromPromise("Server.Provider.complete", "provider", () =>
                flow.complete(payload, callbackUrl),
              ).pipe(
                Effect.mapError((cause) => connectionError("Server.Provider.complete", cause)),
              ),
            method: flow.method,
            providerId: flow.providerId,
            requiredCapabilities: flow.requiredCapabilities,
            start: (continuationId) =>
              fromPromise("Server.Provider.start", "provider", () =>
                flow.start(continuationId),
              ).pipe(Effect.mapError((cause) => connectionError("Server.Provider.start", cause))),
          })),
        ),
...
const asyncDependencies = (options: CreateDomainKitOptions) =>
  Layer.mergeAll(
    Layer.succeed(Identity, {
      authenticate: (request) =>
        fromPromise("Server.Identity.authenticate", "authentication", () =>
          options.identity.authenticate(request),
        ),
    }),
    Layer.succeed(ConnectionPolicy, {
      authorizeReuse: (input) =>
        fromPromise("Server.ConnectionPolicy.authorizeReuse", "authorization", () =>
          options.connectionPolicy.authorizeReuse(input),
        ),
    }),
    asyncProvidersLayer(options.providers),
    Layer.succeed(PendingAuthorizations, {
      consume: (id, now) =>
        fromPromise("Server.PendingAuthorizations.consume", "storage", () =>
          options.pendingAuthorizations.consume(id, now),
        ),
      get: (id, now) =>
        fromPromise("Server.PendingAuthorizations.get", "storage", () =>
          options.pendingAuthorizations.get(id, now),
        ),
      put: (pending) =>
        fromPromise("Server.PendingAuthorizations.put", "storage", () =>
          options.pendingAuthorizations.put(pending),
        ),
    }),
    ManagedDnsConnections.layerFromAsync(options.persistence),
    webCryptoLayer,
  );

const webHandler = <E>(runtime: ManagedRuntime.ManagedRuntime<Handler, E>): WebHandler => ({
  dispose: () => runtime.dispose(),
  fetch: (request) =>
    runtime.runPromise(
      Effect.flatMap(Handler, (handler) => handler.handle(request)),
      {
...
export function toWebHandler<E>(serverLayer: Layer.Layer<Handler, E>): WebHandler {
  return webHandler(ManagedRuntime.make(serverLayer));
}

/**
 * Build a Web handler for async hosts. The host owns persistence setup, migrations, and disposal.
 */
export async function createDomainKit(options: CreateDomainKitOptions): Promise<WebHandler> {
  const runtime = ManagedRuntime.make(
    layer(options).pipe(Layer.provide(asyncDependencies(options))),
  );
  try {
    await runtime.context();
    return webHandler(runtime);
  } catch (cause) {
